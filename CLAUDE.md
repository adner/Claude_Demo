...
- If the user asks you to find bugs you will follow the instructions below. When doing the steps, and executing commands - explain briefly but clearly what you are doing. Especially, describe the results of your operations.
- First, you will use the PAC CLI to retrieve records from the plugintracelog table in Dataverse by running: `pac.exe org fetch --xml "<fetch><entity name='plugintracelog'><attribute name='messageblock'/><attribute name='createdon'/><attribute name='typename'/><attribute name='exceptiondetails'/><order attribute='createdon' descending='true'/></entity></fetch>"`. You will only inspect the most recent record, disregard the others.
...
## Updating the plugin in the sandbox environment
If the user requests it, you can update the plugin in the sandbox environment after the pull request has been approved and merged to main.

- You will first checkout the main branch and pull changes.
- You will then build the plugin.
- You will then run the command 'pac.exe plugin push --pluginId 79557e51-8baf-443f-a0b2-3feb2499bc84 --pluginFile "/TestPlugin/bin/Debug/net462/PluginTest.dll" -t Assembly', which will update the plugin in the sandbox environment.
